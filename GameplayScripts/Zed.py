...
def winstealer_load_cfg(cfg):
    ...
    global spell_priority, combo_key, harass_key, laneclear_key, killsteal_key,smart_combo,autoQKey,use_q_stack,smartCombo
    ...
    rEvade=cfg.get_bool("rEvade",True)
...
def winstealer_save_cfg(cfg):
    ...
    global spell_priority, combo_key, harass_key, laneclear_key, killsteal_key,use_q_stack,autoQKey,smartCombo
    global steal_kill_with_q, e_harass,smart_combo,rSwitch
    global lane_clear_with_q,lane_clear_with_e,flee
...
def simpleMode(game):
    ...
    target2=GetLowestHPTarget(game, 1000)    
    ...
    if use_r_in_combo and IsReady(game, r_spell):
        targetDEAR = GetLowestHPTarget(game, 1500)
        targetR = GetLowestHPTarget(game, 650)
        if targetR:
            if not r_spell.name=="zedr2" :
    ...
    if use_e_in_combo:
        targetLong= GetLowestHPTarget(game, 920)
        targetNear = GetLowestHPTarget(game, 290)
        if IsReady(game, e_spell) and not IsReady(game, w_spell):
            if targetNear:
                e_spell.trigger(False)
...
def LineMode(game):
    ...
    player = game.player
    target = GetLowestHPTarget(game, 700)
    target2 = GetLowestHPTarget(game, 1200)
#------------------------- if No R  ---------------------------    
    if not IsReady(game, r_spell):
        if use_w_in_combo and IsReady(game, w_spell):
...
def SmartMode(game):
    ...
    if use_w_in_combo:
            target = GetLowestHPTarget(game, 2000)
...
###################################  Q mode   ################################################################
    if use_q_in_combo :
        if not w_spell.name=="zedw2" :
            target = GetLowestHPTarget(game, 920)
            ...
            target = GetLowestHPTarget(game, 2500)
            if IsReady(game, q_spell):
                for champ in game.champs:
                    for buff in champ.buffs:
                        if (buff.name == "zedrdeathmark"):
...
def winstealer_update(game, ui):
    ...
    if draw_e_range:
        game.draw_circle_world(game.player.pos, e["Range"], 100, 1, Color.GRAY)
    if draw_r_range:
