...
def winstealer_draw_settings(game, ui):
    ...
    ui.text("Ls-Diana:1.0.0.0")
    ui.text("LifeSaver#3592")
    ui.separator ()
    ui.text("Combo Mode: Save E")
...
def Combo(game):
    ...
    if use_e_in_combo and IsReady(game, e_spell):
            targetE=GetBestTargetsInRange (game,825)
    ...
    if use_w_in_combo and IsReady(game, w_spell) :
            targetW = GetBestTargetsInRange (game,300)
            if targetW :
                    if  game.player.mana >= 60:
    ...
    if use_r_in_combo and IsReady(game, r_spell) :
        targetR=GetBestTargetsInRange(game,425)
        if targetR:
