...
def Combo(game):
    ...
    if target :
        ...
        if smart_combo==1:
            ...
            if getBuff(target,"LeblancQMark"):
                    r_spell.move_and_trigger(game.world_to_screen(target.pos))
            if getBuff (target,"LeblancRQMark")and IsReady (game, w_spell) and use_w_in_combo or getBuff (target, "leblanceroot") :
                if not getBuff(game.player,"LeblancW"):
                    w_spell.move_and_trigger (game.world_to_screen (target.pos))
            if getBuff(target,"LeblancQMark") and not IsReady(game,r_spell) and IsReady (game, w_spell) and use_w_in_combo or getBuff(target,"leblanceroot") :
                if not getBuff(game.player,"LeblancW"):
                    w_spell.move_and_trigger (game.world_to_screen (target.pos))
            if IsReady(game,e_spell) and use_e_in_combo and getBuff(game.player,"LeblancW") and not IsCollisioned(game, target):
                e_spell.move_and_trigger (
                game.world_to_screen (castpoint_for_collision (game, q_spell, game.player, target)))
...
def winstealer_update(game, ui):
    ...
    if game.player.is_alive and game.is_point_on_screen(game.player.pos) and not game.isChatOpen :
        if game.is_key_down(combo_key) and lastQ + 1 < game.time:
            Combo(game)
            lastQ=game.time
        if game.is_key_down(laneclear_key)and lastQ + 1 < game.time:
            LaneClear(game)
            lastQ=game.time