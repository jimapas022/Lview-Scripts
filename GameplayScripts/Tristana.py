...
def winstealer_load_cfg(cfg):
    ...
    w_combo_count = cfg.get_int("w_combo_count", 2)
...
def winstealer_save_cfg(cfg):
    ...
    cfg.set_float("w_combo_count", w_combo_count)
...
def winstealer_draw_settings(game, ui):
    ...
    if ui.treenode("Combo"):
        ...
        w_combo_count = ui.sliderint("E + W Enemy Count", int(w_combo_count), 1, 5)
...
def Laneclear(game):
	...
	if lane_clear_with_e and IsReady(game, e_spell) and game.player.mana > (lcpercentage * game.player.max_mana):
		if (target):
			e_spell.move_and_trigger(game.world_to_screen(target.pos))
		if (minion):
			e_spell.move_and_trigger(game.world_to_screen(minion.pos))
...
def Wtarget(game):
	global wcount
	global w_combo_count, use_w_in_combo
	wcount = 0
	w_spell = getSkill(game, "W")
	for target in game.champs:
		if (target and target.is_visible and target.is_enemy_to(game.player) and target.isTargetable and target.is_alive and game.is_point_on_screen(target.pos) and game.distance(game.player, target) < w["Range"] + game.player.atkRange):
			wcount = wcount + 1
			if (use_w_in_combo and wcount <= w_combo_count and getBuff(target, "tristanaecharge")):
				ecount = getBuff(target, "tristanaecharge").countAlt
				if ecount == 3:
...
def winstealer_update(game, ui):
	...
	e_spell = getSkill(game, "E")
	r_spell = getSkill(game, "R")
	if self.is_alive and draw_w_range:
		game.draw_circle_world(game.player.pos, w["Range"], 100, 1, Color.ORANGE)
	if self.is_alive and draw_e_range:
		game.draw_circle_world(game.player.pos, e["Range"], 100, 1, Color.ORANGE)
	if self.is_alive and draw_r_range:
