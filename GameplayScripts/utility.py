...
def draw_skillshots(game, player):
    ...
    for missile in game.missiles:
        ...
        if spell.flags & SFlag.Line:
            ...
            game.draw_circle_world_filled(curr_pos, missile.width, 20, color2)
...
def draw_champ_world_icon(game, champ, pos, size, draw_distance = False, draw_hp_bar = False, draw_invisible_duration = False, name_ch = False, line_ch = False, minimap_dura = False):
	...
	if draw_distance: #draw_xp_bar
		...
		game.draw_rect_filled(Vec4(pos.x + 1, pos.y + 1, pos.x + 1 + (size - 1)*percent_xp, pos.y + size_xp_bar - 1), Color.BLUE)
		#pos.x += size_hp_bar
		#pos.y += size_hp_bar
...
def show_alert(game, champ):
	...
	draw_champ_world_icon(game, champ, pos, 48.0, True, True, False, True, True)
...
def draw_minimalspellR(game, spell, pos, size, show_lvl = True, show_cd = True):
	...
	for i in range(spell.level):
		offset = i*12
		game.draw_rect_filled(Vec4(pos.x + 3 + offset, pos.y + 11, pos.x + offset + 9, pos.y + 9), Color.DARK_GREEN)
...
def draw_spell(game, spell, pos, size, show_lvl=True, show_cd=True):
    ...
    if show_cd and cooldown > 0.0:
        game.draw_text(pos.add(Vec2(18, 0)), str(int(cooldown)), Color.WHITE)
...
def draw_overlay_on_champ(game, champ):
    ...
    p.x += 39
    p.y -= 24
    draw_spell(game, champ.D, p, 13)
    p.x += 0
    p.y += 17
    draw_spell(game, champ.F, p, 13)
#Vision Tracker
...
def winstealer_load_cfg(cfg):
    ...
    auto_potion=cfg.get_bool("auto_potion",False)
    ...
    skillshots_predict = cfg.get_bool("skillshots_predict", False)
...
def winstealer_save_cfg(cfg):
    ...
    cfg.set_bool("skillshots_predict", False)
...
def winstealer_draw_settings(game, ui):
    ...
    JScolorCyan.a = 1
    #Activator
    ui.text("Xepher Utilities FREE", JScolorPurple)
    ui.text("Updated 3 Sep 2022", JScolorGray)
    ui.text("")
