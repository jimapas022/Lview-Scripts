...
winstealer_script_info = {
    "script": "YM Kassadin",
    "author": "yeetme",
    "description": "YM Kassadin",
    "target_champ": "kassadin",
}
...
mana_e = [60 , 65 , 70 , 75 , 80]
mana_r = [40 , 80 , 160 , 320 , 640]
...
def winstealer_draw_settings(game, ui):
    ...
    ui.text("Author:")
    ui.sameline()
    ui.text("yeetme#4708", JScolorGreen)
    ui.text("==========================")
    ui.text("Version: 1.0.0.1")
    ui.text("==========================")
...
def Combo(game):
    ...
    if use_e_in_combo and IsReady(game, e_spell) and getBuff(game.player, "forcepulsecancast") and game.player.mana > mana_e[game.player.E.level-1]:
    ...
    if use_r_in_combo and IsReady(game, r_spell) and game.player.mana >= 160 :
...
def Jungleclear(game):
    ...
    if jungle_clear_with_w and IsReady(game, w_spell):
        ...
        if target:
            w_spell.trigger(False)
    if jungle_clear_with_e and IsReady(game, e_spell) and  getBuff(game.player, "forcepulsecancast") and  game.player.mana > mana_e[game.player.E.level-1]:
