...
def Healinside(game):
	...
	enemy = GetBestTargetsInRange(game, 1000)#need check if enemy around?
...
def GetNearTarget(game, r_range=1100):
	...
	enemy = GetBestTargetsInRange(game, 1100)
	if enemy:
		for champ in game.champs:
			if (
				champ.is_alive
...
def winstealer_update(game, ui):
	...
	global combo_key, harass_key, laneclear_key, wlist
	r_spell = getSkill(game, "R")
	w_spell = getSkill(game, "W")
	...
	if self.is_alive and not game.isChatOpen:
		...
		if not insided(game, game.player):
			Healoutside(game)
		for champ in game.champs:
			if auto_r and IsReady(game, r_spell):
				meow(game, champ)
