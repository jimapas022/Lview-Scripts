...
def items_in_list(itms, lst) -> bool:
    return True in [x in lst for x in itms]


def is_immortal(target) -> bool:
    return items_in_list(["willrevive", "ChronoShift", "UndyingRage", "TaricR", "KindredRNoDeathBuff"],
                        [buff.name for buff in target.buffs])
...
def steal_with_r(game):
    ...
    if ValidTarget(target) and not target_airborned(game, target) and not is_immortal(target):
