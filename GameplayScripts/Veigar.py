...
def Combo(game):
    ...
    if ewq:
        if use_e_in_combo and IsReady(game, e_spell):
            targetE=GetBestTargetsInRange (game,1000)
            
            if targetE:
                # for buff in targetE.buffs:
                #         print(buff.name)
                e_travel_time=2000/2200
                ...
                if game.player.pos.distance (predicted_target.pos) <= 2000:
        ...
        if use_w_in_combo and IsReady(game, w_spell) :
            targetW = GetBestTargetsInRange (game,900)
            if targetW :
                w_travel_time = 2000/ 1800
                ...
                if game.player.pos.distance (predicted_target.pos) <= 2000 and getBuff(targetW, "veigareventhorizonstun"):
        ...
        if use_q_in_combo and IsReady(game, q_spell) :
            ...
            if targetQ :
                ...
                if game.player.pos.distance (predicted_target.pos) <= 1500 and not IsCollisioned(game, targetQ):
