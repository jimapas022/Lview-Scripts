...
class Slot:
	''' 
		Spell slot indices. Usage: Unit_Champion.spells[Slot.Q] 
	'''
	Q = 0
	W = 1
	E = 2
...
class SpellPredictor:
	def _ui(self, game, ui):
		pass
	
	def _predict(self, game, player, target, spell):
		return game.predict_cast_point(player, target, spell)
...
class RSpell:
	...
	def check_condition(self, game, player, target, spell):
		if not self.condition:
			return True
			
		return self.condition.check(game, player, target, spell)
...
class SpellCondition:
	...
	def check(self, game, player, target, spell):
		if self.enabled:
			return self._check(game, player, target, spell)
		return True

	def _check(self, game, player, target, spell):
		return True
...
class SpellRotation:
	...
	def find_spell(self, game, target_selector, target_extractor):
		'''
		...
		for i, rspell in enumerate(self.rotation_spells):
			...
			target = target_selector.get_target(game, target_extractor(game, player, spell))
			if not target:
				continue
				
			if not rspell.check_condition(game, player, target, spell):
				continue
	...
	def get_spell(self, game, player, target_selector, target_extractor):
		if player.curr_casting and player.curr_casting.remaining > 0.0:
			if player.curr_casting.name not in self.IgnoreCurrentCast:
				return None, None
			
		target, spell, rspell = self.find_spell(game, target_selector, target_extractor)
		if not spell:
			return None, None
		
		point = rspell.predictor._predict(game, player, target, spell)
		if not point:
			return None, None
		
		return spell, point
		
	def cast(self, game, target_selector, target_extractor):
		player = game.player
		now = time()
		if now < self.pause_until:
			return
			
		spell, point = self.get_spell(game, player, target_selector, target_extractor)
		if spell:
			...
			if spell.static.has_flag(Spell.ChargeableSkill):
				...
				if channel_start_time:
					...
					if should_end and chargeable_info.dist_calculator(dt) > dist_to_target:
						game.end_channel(spell, point)
						self.chargeables[spell.name] = None
						
				elif game.start_channel(spell):
					self.chargeables[spell.name] = now
			else:
				if game.cast_spell(spell, point):
	...
	def ui(self, game, ui):
		...
		for i in range(len(self.mask)):
			...
			spell = game.player.spells[slot]
			ui.image(spell.static.icon if spell.static else 'None', Vec2(16, 16), Color.White)
			ui.sameline()
			if ui.beginmenu(slot_str + ' ' + spell.name):
				self.mask[i] = ui.checkbox('Use spell', self.mask[i])
				self.rotation_spells[i].ui(game, ui)
				ui.endmenu()
