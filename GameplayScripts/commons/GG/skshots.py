...
class Spell:
    def __init__(
        self, name, missile_names, flags, delay=0.0, danger=DangerLevels.Fastest
    ):
        global MissileToSpell, SpellsToEvade, Spells
...
ChampionSpells = {
    ...
    "ekko": [
        ...
        Spell("ekkow", ["ekkowmis"], SFlag.Area, 0.0, DangerLevels.Fastest),
    ...
    "lucian": [
        Spell("lucianq", ["lucianqmis"], SFlag.SkillshotLine, 0.4, DangerLevels.Fastest),
    ...
    "lux": [
        ...
        Spell("luxmalicecannon", ["luxrmis"], SFlag.Line),
    ],
    "nautilus": [
        Spell(
            ...
            SFlag.SkillshotLine,
            0.25,
            DangerLevels.Fastest | DangerLevels.UseSpell,
        )
    ],
    ...
    "veigar": [
            Spell("veigarbalefulstrike", ["veigarbalefulstrikemis"], SFlag.Line),
            Spell("veigarbaledarkmattercastlockout", ["veigarbaledarkmattercastlockout"], SFlag.Area),
            Spell("veigareventhorizon", ["veigareventhorizon"], SFlag.Area),
            Spell("veigarr", ["veigarr"], SFlag.Targeted),
    ],
...
def get_active_parent_spell(spell_name):
    global SpellsToEvade, Spells
    return SpellsToEvade.get(spell_name, None)
...
def InMissileSkillShot(game, pos, missile, spell, radius):
...
def InNotMissileSkillShot(game, pos, missile, spell, radius):
    pointSegment, pointLine, isOnSegment = VectorPointProjectionOnLineSegment(
        missile.start_pos, missile.end_pos, pos
    )
    if spell.flags & SFlag.Area:
...
def InSpellSkillShot(game, pos, aSpell, spell, radius):
    pointSegment, pointLine, isOnSegment = VectorPointProjectionOnLineSegment(
        aSpell.spell_start_pos, aSpell.spell_end_pos, pos
    )
    if spell.flags & SFlag.Line or spell.flags & SFlag.SkillshotLine:
        return (
            isOnSegment
...
def InNotSpellSkillShot(game, pos, aSpell, spell, radius):
    pointSegment, pointLine, isOnSegment = VectorPointProjectionOnLineSegment(
        aSpell.spell_start_pos, aSpell.spell_end_pos, pos
    )
    if spell.flags & SFlag.Cone:
    ...
    if spell.flags & SFlag.Area:
        return game.point_on_line(
            game.world_to_screen(aSpell.spell_start_pos),
            game.world_to_screen(aSpell.spell_end_pos),
            game.world_to_screen(pos),
            radius,
        )
    return (
        isOnSegment
        and pointSegment.distance(pos)
        <= (aSpell.width or aSpell.cast_radius) + radius + game.player.gameplay_radius
    )


def IsDanger(game, point):
    for missile in game.missiles:
        if not missile or not game.player.is_alive or missile.is_ally_to(game.player):
            continue
        ...
        if not spell:
            continue
        if InMissileSkillShot(game, point, missile, spell, game.player.gameplay_radius):
            return True
        else:
            return False
...
def getEvadePosMissile(game, current, br, missile, spell):
    self = game.player
...
def getEvadePosSpell(game, current, br, missile, spell):
    self = game.player
