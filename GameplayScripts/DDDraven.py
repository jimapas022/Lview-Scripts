...
def Combo(game):
    ...
    global draw_q_range, draw_w_range, draw_e_range, draw_r_range
    global combo_key, laneclear_key,forceAXE
    ...
    if use_q_in_combo:
        targetAXE=GetBestTargetsInRange(game,800)
        if targetAXE:
            
            for missle in game.missiles:
                if missle.name=="dravenspinningreturncatch"  or missle.name=="dravenspinningreturn" :
    ...
    if use_e_in_combo and IsReady(game, e_spell) :
                targetR=GetBestTargetsInRange(game,1050)
                if targetR:
    ...
    if use_w_in_combo and IsReady(game, w_spell) :
                targetW = GetBestTargetsInRange (game,800)
                if targetW :
...
def winstealer_update(game, ui):
    ...
    #     if missle.name=="dravenspinningreturncatch":
    #         if forceAXE:
